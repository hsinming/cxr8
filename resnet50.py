...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
logging.basicConfig(filename=os.path.join(log_dir, "resnet50log.log"), filemode='w', level=logging.INFO, format='%(message)s')
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloders[phase]:
                ...
                if phase == 'train': tmpIter = iter_size
                else: tmpIter = 1
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.model_ft.layer4(x)#2048x32x32

        #x = self.model_ft.avgpool(x)

        x = self.transition(x)#2048x16x16
        x = self.globalPool(x)
        x = x.view(x.size(0), -1)
...
if __name__ == '__main__':
    ...
    plt.xlabel("iter")
    plt.legend()
    fig.savefig(os.path.join(statistic_dir, "resnet50.png"))
